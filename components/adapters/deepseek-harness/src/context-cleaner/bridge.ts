/**
 * DSH ContextCleanerHostBridge.
 *
 * Implements the cleaner host contract for DeepSeek Harness: enumerate sessions,
 * build a clean snapshot from the LIVE canonical surface (DSH is its own truth
 * source — no separate snapshot store), and delegate the analyzed→approved→
 * scheduled state machine to the shared control plane. The actual canonical
 * rewrite of a scheduled clean is the execution bridge (runtime.ts, reusing R4);
 * this bridge only takes the plan through to `scheduled`.
 *
 * The validation here mirrors the claude-code/codex bridges: user approvals are
 * checked structurally and receipts are re-validated against the immutable plan.
 */
...
import { DSH_CLEANER_SCHEDULE_SCHEMA, writeDshCleanerSchedule } from "./scheduler.js";
...
function normalizedUniqueStrings(values: unknown): string[] | undefined {
  ...
  return normalized.every(Boolean) && new Set(normalized).size === normalized.length ? normalized : undefined;
}
...
function validReceiptState(receipt: ContextCleanReceipt): boolean {
  ...
  if (receipt.status === "applied") {
    ...
    return receipt.fallbackUsed === false
      ...
      && itemIds !== undefined && itemIds.length > 0;
  }
  ...
  return !["analyzed", "approved", "scheduled"].includes(receipt.status) || receipt.fallbackUsed === false;
}

function validateApprovedRequest(request: ExecuteApprovedContextCleanParams): string[] {
  if (request.schemaVersion !== CONTEXT_CLEAN_SCHEMA_VERSION) throw new Error("dsh_clean_approval_schema_mismatch");
  if (request.hostId !== DSH_HOST_ID) throw new Error("dsh_clean_approval_host_mismatch");
  if (typeof request.cleanPlanId !== "string" || !request.cleanPlanId.trim()
    || typeof request.sessionId !== "string" || !request.sessionId.trim()
    || typeof request.baseRevision !== "string" || !request.baseRevision.trim()
    || !canonicalTimestamp(request.approvedAt)
    || !Array.isArray(request.selectedTasks) || request.selectedTasks.length === 0) {
    throw new Error("dsh_clean_approval_invalid");
  }
  const taskIds = normalizedUniqueStrings(request.selectedTasks.map((task) => (isRecord(task) ? task.taskId : undefined)));
  if (!taskIds) throw new Error("dsh_clean_approval_invalid");
  const claimedItemIds = new Set<string>();
  for (const task of request.selectedTasks) {
    if (!isRecord(task)) throw new Error("dsh_clean_approval_targets_invalid");
    const itemIds = normalizedUniqueStrings(task.itemIds);
    const itemDigests = isRecord(task.itemDigests) ? task.itemDigests : undefined;
    if (!itemIds || itemIds.length === 0 || !itemDigests || Object.keys(itemDigests).length !== itemIds.length) {
      throw new Error("dsh_clean_approval_targets_invalid");
    }
    for (const itemId of itemIds) {
      if (claimedItemIds.has(itemId) || typeof itemDigests[itemId] !== "string" || !itemDigests[itemId]!.trim()) {
        throw new Error("dsh_clean_approval_targets_invalid");
      }
...
function validateReceipt(params: {
  ...
  if (!isRecord(params.receipt)) throw new Error("dsh_clean_receipt_mismatch");
  ...
  if (receipt.schemaVersion !== CONTEXT_CLEAN_SCHEMA_VERSION
    ...
    throw new Error("dsh_clean_receipt_mismatch");
  }
...
export function createDshContextCleanerBridge(params: {
  ...
  return {
    ...
    async readCleanSnapshot(sessionId): Promise<ContextCleanSnapshot> {
      ...
      return buildDshCleanSnapshot({ session, registry, revision: surfaceRevision(session) }).snapshot;
    },
    async executeApprovedClean(request) {
      const selectedTaskIds = validateApprovedRequest(request);
      // Control plane drives analyzed→approved→scheduled via the stateDir stores.
      // The scheduled clean's actual canonical apply is the execution bridge
      // (runtime.ts), which reuses R4 inside DSH's request path.
      ...
      if (receipt.status === "scheduled") {
        // Record the session→plan pointer so observability and the apply-trigger
        // can find this scheduled plan by session (read-by-id stores can't).
        await writeDshCleanerSchedule({
    ...
    async readCleanReceipt(planId) {
      if (!planId.trim()) throw new Error("dsh_clean_plan_id_invalid");
    ...
    async cancelCleanPlan(planId) {
      if (!planId.trim()) throw new Error("dsh_clean_plan_id_invalid");
      return validateReceipt({ receipt: await params.controlPlane.cancelCleanPlan(planId), planId });
    },
  };
}
